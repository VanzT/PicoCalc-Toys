      ...
      IF LEFT$(t$,5) = "HELLO" AND assigned% = 0 THEN
        ...
        ELSE
          ...
          IF myTicket% > peerTicket% THEN
            ...
            TEXT W\2, H\2 + 16, "B = Brown", "CT", 1, 1, RGB(160,100,50),  bgColor
            chosen% = 0
            DO
              k$ = INKEY$
...
' Poll for an incoming packet and dispatch it
SUB NetPoll()
  LOCAL msg$
  msg$ = MM.MESSAGE$
...
SUB DoOpeningExchange
  ...
  ' Choose and send our die once, then resend every second until we have both
  DO
    NetPoll
    now% = NowMs%()
...
SUB ApplyGameOver(winner$)
  ...
  IF myRole$ = "WHITE" THEN
    IF gameWinner$ = "WHITE" THEN
      TEXT W\2, H\2, "YOU WIN", "CT", 1, 3, RGB(255,220,0), bgColor
    ELSE
      TEXT W\2, H\2, "YOU LOSE", "CT", 1, 3, RGB(255,255,255), bgColor
    ENDIF
  ELSE
