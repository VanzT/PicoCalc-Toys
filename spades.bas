...
NetPair
CheckLoadGame   ' check for save file and handle load/new choice
...
ShowGameOver
' Delete save file so a finished game can't be reloaded
IF DIR$("B:spades.sav") <> "" THEN KILL "B:spades.sav"
WEB UDP CLOSE
LED_Off : CHAIN "B:menu.bas"
...
SUB NetPair
  ...
  DO
    ...
    IF INKEY$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
    PAUSE 10
...
SUB PickVariant
  ...
  IF myRole% = 1 THEN
    ...
    DO
      ...
      IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
      PAUSE 10
    LOOP UNTIL chosen% = 1
...
SUB DoSetup
  ...
  LOCAL k$, msg$, i%, p%, nxt%, idx%, keptAlready%, whoseTurn%, gotDone%, setupComma%
  ...
  FOR setupTurn% = 1 TO 26
    ...
    IF whoseTurn% = myRole% THEN
      ...
      IF faceUpDiscard% THEN
        lastMsg$ = "SETUP " + STR$(discarded%) + "," + STR$(setupTurn%)
      ELSE
        lastMsg$ = "SETUP 0," + STR$(setupTurn%)
      END IF
      WEB UDP SEND peer$, PORT%, lastMsg$
      LED_Off
      ...
      DO
        ...
        IF MM.MESSAGE$ <> "" AND MM.MESSAGE$ <> lastRcvd$ THEN
          ...
          IF LEFT$(msg$,5) = "SETUP" THEN
            setupComma% = INSTR(msg$, ",")
            IF setupComma% > 0 THEN
              discarded% = VAL(MID$(msg$, 7, setupComma%-7))
            ELSE
...
SUB DoBidding
  ...
  IF myRole% = 2 THEN
    ...
    ' Wait for opponent bid — auto-resend every 2s in case packet lost
    DrawBidWait
    bidTimer! = TIMER
    DO
      k$ = INKEY$
      IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
      IF TIMER - bidTimer! > 2000 THEN
        WEB UDP SEND peer$, PORT%, lastMsg$
        bidTimer! = TIMER
      END IF
      IF MM.MESSAGE$ <> "" AND MM.MESSAGE$ <> lastRcvd$ THEN
  ...
  ELSE
    ' Wait for Second Player's bid first — auto-resend our last msg every 2s
    LED_Off
    DrawBidWait
    bidTimer! = TIMER
    DO
      k$ = INKEY$
      IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
      IF TIMER - bidTimer! > 2000 AND lastMsg$ <> "" THEN
        WEB UDP SEND peer$, PORT%, lastMsg$
        bidTimer! = TIMER
      END IF
      IF MM.MESSAGE$ <> "" AND MM.MESSAGE$ <> lastRcvd$ THEN
...
' ============================================================
SUB DoScoring
  LOCAL myRound%, oppRound%, myBidStr$, oppBidStr$, k$, gotNext%, resendTimer!
  ...
  CLS BG%
  TEXT ScrW%\2, 20,  "Hand " + STR$(handNum%) + " complete",                      "CT", 1, 2, WHITE%, BG%
  TEXT ScrW%\2, 65,  "You:  bid " + myBidStr$  + "  took " + STR$(myTricks%),     "CT", 1, 1, WHITE%, BG%
  TEXT ScrW%\2, 83,  "Opp:  bid " + oppBidStr$ + "  took " + STR$(oppTricks%),    "CT", 1, 1, CYAN%,  BG%
  TEXT ScrW%\2, 108, "Your score: " + STR$(myScore%) + "   bags: " + STR$(myBags%),  "CT", 1, 1, WHITE%, BG%
  TEXT ScrW%\2, 126, "Opp score:  " + STR$(oppScore%) + "   bags: " + STR$(oppBags%), "CT", 1, 1, CYAN%,  BG%
  TEXT ScrW%\2, 170, "Press any key to continue", "CT", 1, 1, WHITE%, BG%
  ' Auto-save after every hand
  SaveGame
  TEXT ScrW%\2, 186, "Game saved.", "CT", 1, 1, RGB(0,220,0), BG%
  ...
  ' IMPORTANT: clear lastRcvd$ AFTER pressing key, not before.
  ' If we clear it before, a NEXTHAND already in the buffer gets
  ' filtered as a duplicate on the first check. Instead we clear
  ' it here and use a periodic resend so neither side can hang.
  lastRcvd$ = ""
  lastMsg$  = "NEXTHAND"
  WEB UDP SEND peer$, PORT%, lastMsg$
  ...
  DO
    k$ = INKEY$
    IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
    ' Periodic resend every 2 seconds in case packet was lost
    IF TIMER - resendTimer! > 2000 THEN
      WEB UDP SEND peer$, PORT%, lastMsg$
      resendTimer! = TIMER
    END IF
    IF MM.MESSAGE$ <> "" AND MM.MESSAGE$ <> lastRcvd$ THEN
...
' ============================================================
'  WIN/LOSS RECORD  (B:spades.won — device-specific)
'  Format: two lines:  wins: N
'                      losses: N
' ============================================================
SUB UpdateWonFile
...
' ============================================================
'  SAVE / LOAD
'  Save file: B:spades.sav  (one file, always overwritten)
'  Format (one value per line):
'    Line 1: magic number (19641)
'    Line 2: faceUpDiscard%
'    Line 3: myScore%
'    Line 4: oppScore%
'    Line 5: myBags%
'    Line 6: oppBags%
'    Line 7: myRole%  (role for NEXT hand, after swap already applied)
'    Line 8: handNum%
' ============================================================


SUB SaveGame
  OPEN "B:spades.sav" FOR OUTPUT AS #1
  PRINT #1, faceUpDiscard%
  PRINT #1, myScore%
  PRINT #1, oppScore%
  PRINT #1, myBags%
  PRINT #1, oppBags%
...
SUB LoadGame
  OPEN "B:spades.sav" FOR INPUT AS #1
  INPUT #1, faceUpDiscard%
  INPUT #1, myScore%
  INPUT #1, oppScore%
  INPUT #1, myBags%
  INPUT #1, oppBags%
...
' -- After pairing: check for save file, handle load/new ------
SUB CheckLoadGame
  LOCAL k$, msg$, hasSave%, theirChoice$, myChoice$, peerHasSave%

  ' Check if save file exists
  hasSave% = 0
  IF DIR$("B:spades.sav") <> "" THEN hasSave% = 1

  ' Exchange save status with peer
  lastRcvd$ = ""
  IF hasSave% THEN
    WEB UDP SEND peer$, PORT%, "HASSAVE 1"
  ELSE
    WEB UDP SEND peer$, PORT%, "HASSAVE 0"
  END IF

  ' Wait for peer's status
  peerHasSave% = -1
  DO
    k$ = INKEY$
    IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
    IF MM.MESSAGE$ <> "" AND MM.MESSAGE$ <> lastRcvd$ THEN
      msg$      = MM.MESSAGE$
      lastRcvd$ = msg$
      IF LEFT$(msg$,7) = "HASSAVE" THEN peerHasSave% = VAL(MID$(msg$,9))
    END IF
    PAUSE 10
  LOOP UNTIL peerHasSave% >= 0

  ' If neither has a save, go straight to variant pick
  IF hasSave% = 0 AND peerHasSave% = 0 THEN
    PickVariant
    EXIT SUB
  END IF

  ' At least one player has a save — show load/new screen
  CLS BG%
  TEXT ScrW%\2, 40,  "Saved game found!", "CT", 1, 2, WHITE%, BG%
  IF hasSave% = 0 THEN
    TEXT ScrW%\2, 80, "(you have no save file)", "CT", 1, 1, GREY%, BG%
  END IF
  TEXT ScrW%\2, 115, "L = Load saved game",  "CT", 1, 1, WHITE%, BG%
  TEXT ScrW%\2, 135, "N = Start new game",   "CT", 1, 1, WHITE%, BG%
  ...
  DO
    k$ = INKEY$
    IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
    IF k$ = "L" OR k$ = "l" THEN myChoice$ = "LOAD"
    IF k$ = "N" OR k$ = "n" THEN myChoice$ = "NEW"
  LOOP UNTIL myChoice$ <> ""

  lastRcvd$ = ""
  WEB UDP SEND peer$, PORT%, "CHOICE " + myChoice$

  ' Wait for peer's choice
  theirChoice$ = ""
  TEXT ScrW%\2, 175, "Waiting for opponent...", "CT", 1, 1, GREY%, BG%
  DO
    k$ = INKEY$
    IF k$ = CHR$(KEY_Q%) THEN WEB UDP CLOSE : LED_Off : CHAIN "B:menu.bas"
    IF MM.MESSAGE$ <> "" AND MM.MESSAGE$ <> lastRcvd$ THEN
      msg$      = MM.MESSAGE$
      lastRcvd$ = msg$
      IF LEFT$(msg$,6) = "CHOICE" THEN theirChoice$ = MID$(msg$,8)
    END IF
    PAUSE 10
  LOOP UNTIL theirChoice$ <> ""

  ' If either chose NEW — start fresh, delete save files
  IF myChoice$ = "NEW" OR theirChoice$ = "NEW" THEN
    CLS BG%
    TEXT ScrW%\2, ScrH%\2, "Starting new game", "CT", 1, 1, WHITE%, BG%
    PAUSE 1200
    PickVariant
    EXIT SUB
  END IF
  ...
  IF hasSave% THEN
    LoadGame
    ' Save stores role for the hand just completed — swap for next hand
    myRole% = 3 - myRole%
    CLS BG%
    TEXT ScrW%\2, ScrH%\2 - 10, "Game loaded!", "CT", 1, 2, WHITE%, BG%
    TEXT ScrW%\2, ScrH%\2 + 20, "Resuming hand " + STR$(handNum%+1), "CT", 1, 1, GREY%, BG%
    PAUSE 1500
  ELSE
    ' I have no save but peer does — peer's state is authoritative
    ' My scores will be wrong; peer needs to send them
    CLS BG%
    TEXT ScrW%\2, ScrH%\2, "No save file found!", "CT", 1, 1, YELLOW%, BG%
    TEXT ScrW%\2, ScrH%\2+20, "Starting new game instead", "CT", 1, 1, WHITE%, BG%
    PAUSE 1500
    PickVariant
    EXIT SUB
  END IF

  ' Loaded — skip variant pick (it's in the save)
  ' myRole% is already set correctly for the next hand
END SUB
