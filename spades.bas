...
SUB DoScoring
  ...
  TEXT ScrW%\2, 186, "S = Save  (both players should save)", "CT", 1, 1, WHITE%, BG%
  ' Flush any buffered keypresses from play phase
  DO : LOOP UNTIL INKEY$ = ""
  ' Wait for deliberate keypress — S saves, anything else continues
  DO
    PAUSE 10
    k$ = INKEY$
    IF k$ = "S" OR k$ = "s" THEN
      SaveGame
      TEXT ScrW%\2, 202, "Game saved.", "CT", 1, 1, RGB(0,220,0), BG%
      PAUSE 1000
    END IF
    IF k$ <> "" AND k$ <> "S" AND k$ <> "s" THEN EXIT DO
  LOOP
